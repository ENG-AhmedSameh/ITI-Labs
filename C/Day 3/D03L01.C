...
void saveEmployeeData(char id[],char name[],int salary,int bouns,int deduct){
    ...
    char c,d;
    for (i=0;i<50;i++){
        emp[currentEmployee].ID[i] = id[i];
        emp[currentEmployee].Name[i] = name[i];
    }
    emp[currentEmployee].Salary = salary;
    emp[currentEmployee].Bouns = bouns;
    emp[currentEmployee].Deduct = deduct;
    currentEmployee++;
}
...
void displayEmployeesData(){
    ...
    do{
        ...
        printf("%s",emp[i].Name);
        gotoxy(43,14);
        netSalary = emp[i].Salary + emp[i].Bouns - emp[i].Deduct;
